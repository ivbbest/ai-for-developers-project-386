...
- Dev-режим фронта: Vite-proxy `/api` → бэкенд (CORS не нужен). Требование курса
  ...
  (dev-связка бэк+фронт): на этапе 2 бэкенда ещё нет, фронт
  работает против стаба контракта (`contract/mock-server`) — требование к нему
  неприменимо (N6).
...
- План работ по этапам выполнен и удалён из репозитория; следы — в истории git
  и `.agents/tasks.md`; разбор превентивных проверок —
