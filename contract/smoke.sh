#!/usr/bin/env bash
# Smoke контракта (шаг 1.6): Prism-мок по contract/dist/openapi.yaml + curl
# по всем 5 ручкам и кодам 200/201/400/404/409. 404/409 — через заголовок
...
# Prism 5.16 игнорирует относительный servers.url ("/api") и монтирует пути
# с корня; сам префикс — часть контракта и проверяется отдельно по openapi.yaml.
BASE="http://127.0.0.1:${PORT}"
BODY=/tmp/prism-smoke-body
FAILS=0
...
check_body() {
  ...
  if grep -q "$pattern" "$BODY"; then
    echo "PASS  $name (body has $pattern)"
  else
...
if [ "$ready" != 1 ]; then
  echo "FAIL  Prism не поднялся на :$PORT"
  cat /tmp/prism-smoke.log
  exit 1
fi

# C1: префикс /api зафиксирован в контракте (Prism его не монтирует — см. выше)
if grep -q 'url: /api' dist/openapi.yaml; then
...
check_body "201 — id брони" '"id"'
check "POST /bookings невалидный → 400" 400 -H 'Content-Type: application/json' -d "$BAD_BOOK" "$BASE/bookings"
...
# Prism без состояния: ответ генерируется по схеме, а не эхом запроса —
# проверяем наличие обязательных полей EventType.
check_body "201 типа — поля id/title/durationMinutes" '"durationMinutes"'
