...
- [ ] 4.1 Playwright: webServer поднимает бэк+фронт; полный сценарий + «слот стал Занято»,
      повторная запись → ошибка (409), создание типа; зелёный локально в контейнере с браузерами
